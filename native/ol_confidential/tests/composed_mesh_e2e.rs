...
fn verify_threshold_recovery() {
    ...
    if let Ok(garbage) = reconstruct_bytes(&xs_short, &short_streams, 3) {
        assert_ne!(garbage, secret, "K-1 shares must not recover the secret");
    }
}

fn verify_active_routing(phone_id: [u8; DEVICE_ID_LEN], laptop_id: [u8; DEVICE_ID_LEN]) {
...
fn verify_software_attestation() {
    ...
    let sealed = provider.seal_master(&[0xAB; 32]).unwrap();
...
fn verify_duress_unlock() {
    let real_plaintext = b"the truth: this is the real master notes payload";
    let decoy_plaintext = b"decoy content: looks plausible at first glance";
    let real_code = b"open-sesame-real-2026";
    let decoy_code = b"open-sesame-decoy-2026";
    let field_witness = [0u8; 32];
    let envelope = create_duress_envelope(
        real_plaintext,
        decoy_plaintext,
        real_code,
        decoy_code,
    ...
    .unwrap();
    let outcome = unlock_duress_envelope(&envelope, real_code, Some(&field_witness)).unwrap();
    match outcome {
        UnlockOutcome::Real(plaintext) => assert_eq!(plaintext, real_plaintext),
        other => panic!("expected Real unlock, got {other:?}"),
    }
}

fn verify_rows_touched() {
    let rows_touched = [1, 5, 6, 7, 8, 9, 10].into_iter().collect::<BTreeSet<_>>();
...
fn composed_mesh_walks_all_ten_rows() {
    ...
    .unwrap();
    let phone_verifier = HybridVerifyingKey::from_bytes(&phone_att.subkey_vk_bytes).unwrap();
    let mut table = RouteTable::empty();
    table.ingest(ann, &phone_verifier).unwrap();
    ...
    verify_threshold_recovery();
    verify_active_routing(phone_id, laptop_id);
    verify_software_attestation();
    verify_duress_unlock();
    verify_rows_touched();
}

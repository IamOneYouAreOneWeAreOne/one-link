...
fn make_source(seed: u64, len: usize) -> Vec<u8> {
    ...
    let mut state = seed.wrapping_add(0x00C0_FFEE);
    while out.len() < len {
...
fn bench_encode_one_symbol(c: &mut Criterion) {
    ...
    for &k in &[8u32, 64, 256] {
        let source = make_source(
            0xDEAD_BEEF,
            usize::try_from(k).expect("benchmark K fits in usize") * SYMBOL_LEN,
        );
        let enc = LtEncoder::new(&source, SYMBOL_LEN).unwrap();
        group.throughput(Throughput::Bytes(
            u64::try_from(SYMBOL_LEN).expect("symbol length fits in u64"),
        ));
...
fn bench_decode_full_chunk(c: &mut Criterion) {
    ...
    for &k in &[8u32, 64, 256] {
        ...
        let symbols: Vec<_> = (0u32..max_symbols)
            ...
            .collect();
        group.throughput(Throughput::Bytes(
            u64::from(k) * u64::try_from(SYMBOL_LEN).expect("symbol length fits in u64"),
        ));
        group.bench_with_input(BenchmarkId::from_parameter(k), &k, |b, _| {
            b.iter(|| {
...
fn bench_packet_round_trip(c: &mut Criterion) {
    ...
    group.throughput(Throughput::Bytes(
        u64::try_from(SYMBOL_LEN + 44).expect("packet benchmark size fits in u64"),
    ));
    group.bench_function("encode", |b| {
        b.iter(|| {
            ...
            black_box(p.encode().unwrap())
        });
    });
    let encoded = FountainPacket::new(chunk_id, 64, 7, 64 * 1024, payload.clone())
        .encode()
        .unwrap();
...
mod criterion_benchmark_harness {
    use super::{bench_decode_full_chunk, bench_encode_one_symbol, bench_packet_round_trip};

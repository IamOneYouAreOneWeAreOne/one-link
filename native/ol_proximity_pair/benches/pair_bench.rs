...
use ol_proximity_pair::{
    block_syndrome, privacy_amplify, quantize_observations,
    reconcile_with_syndrome, QuantizeConfig,
};
...
fn bench_quantize(c: &mut Criterion) {
    ...
    c.bench_function("quantize/512_bytes", |bch| {
        bch.iter(|| {
            black_box(quantize_observations(black_box(&obs), &cfg).unwrap())
        });
    });
}
...
fn bench_amplify(c: &mut Criterion) {
    ...
    c.bench_function("amplify/256_bits_blake3", |bch| {
        bch.iter(|| {
            black_box(privacy_amplify(black_box(&bits), black_box(&salt)))
        });
    });
}

...
use ol_device_mesh::{
    mint_subkey, sibling_witness, state_root, verify_liveness, DeviceClass,
    LivenessProof, MasterIdentity, DEFAULT_LIVENESS_SKEW_SECS,
};
use rand::rngs::OsRng;
...
fn liveness_verify_constant_time_across_tamper_positions() {
    ...
    eprintln!(
        "liveness-verify timing totals (ns) = {totals:?}, rel_stddev = {rel:.4}"
    );

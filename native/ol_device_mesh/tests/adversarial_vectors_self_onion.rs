...
use ol_device_mesh::self_onion::{
    ...
    sign_onion_attestation, OnionIdentity, OnionKeyRegistry, SelfOnionPeelOutcome,
};
...
fn adversarial_attestation_under_different_master_rejected() {
    ...
    let att = sign_onion_attestation(
        &master_a, id, identity.public_bytes(), 0, 365,
    )
    .unwrap();
...
fn adversarial_registry_day_out_of_window_rejected() {
    ...
    let identity = derive_onion_identity(&master, &id);
    let att = sign_onion_attestation(
        &master, id, identity.public_bytes(), 10, 100,
    )
    .unwrap();
    let mut reg = OnionKeyRegistry::empty();
    reg.ingest(att, &master.verifying_key()).unwrap();
...
fn adversarial_circuit_single_hop_rejected() {
    ...
    let err = build_self_onion_circuit(&route, &reg, 0, b"hi", &mut OsRng)
        .unwrap_err();
    assert!(matches!(err, DeviceMeshError::SelfOnionRouteTooShort { .. }));
}
...
fn adversarial_circuit_unknown_hop_rejected() {
    ...
    let att = sign_onion_attestation(
        &master, ids[0], identity_known.public_bytes(), 0, 365,
    )
    .unwrap();
    reg.ingest(att, &master.verifying_key()).unwrap();
    ...
    let err =
        build_self_onion_circuit(&route, &reg, 0, b"hi", &mut OsRng).unwrap_err();
    assert!(matches!(
        err,
        DeviceMeshError::OnionRegistryDeviceMissing { .. }
    ));
...
fn adversarial_peel_with_wrong_identity_rejected() {
    ...
    let packet = build_self_onion_circuit(
        &route, &reg, 0, b"target ids[1]", &mut OsRng,
    )
    .unwrap();
...
fn adversarial_packet_carrying_non_self_onion_payload_dropped() {
    ...
    use ol_onion::sphinx::core::{
        build_sphinx_onion, generate_static_keypair, SphinxHop,
    };
    ...
    let packet =
        build_sphinx_onion(&eph_sk, &[hop], b"this is NOT self-onion", &mut OsRng)
            .unwrap();
...
fn adversarial_circuit_payload_oversize_rejected() {
    ...
    let err =
        build_self_onion_circuit(&route, &reg, 0, &big, &mut OsRng).unwrap_err();
    assert!(matches!(
        err,

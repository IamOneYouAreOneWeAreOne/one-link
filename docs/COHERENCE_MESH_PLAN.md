...
|---|---|---|---|
| 1 | **Post-quantum identity** | 🟡 PQ-hybrid KEM shipped; PQ signatures NEW | Ed25519 + Dilithium hybrid master key; ML-KEM-768 + X25519 hybrid per-device KEM; hardware-bound (Secure Enclave / StrongBox / TPM) where available; TOFU-degrading where not. Survives quantum. |
| 2 | **Pair-by-QR + channel-reciprocity 2FA** | 🟡 PARTIAL (harvest OneField bootstrap.cl) | In-person QR scan + Ed25519 challenge + visual SAS + chain-key derivation. Optional Factor 2: RF channel reciprocity proof-of-presence so even captured QR codes can't be remote-relayed. Network is never trusted during pair. |
| 3 | **Sovereign discovery** | 🟡 PARTIAL (harvest OneField discovery.cl scaffold) | DHT signed by identity, gossiped peer-to-peer. STUN via paired peers (no third-party endpoints). Per-protocol announce cadence (WiFi 0.1s, LE 0.05s, LoRa 30s) already production-tested. No DNS, no rendezvous server, no central registry. |
...
| 5 | **Onion circuits** | ❌ NEW BUILD | 1-hop (pinned-contacts default) and 3-hop (paranoid mode) onion circuits through paired peers. Each hop only knows previous + next. Path selection via the Phase E coherence field — prefers high-τ_c relays. Nested AEAD per hop. |
| 6 | **Cover traffic** | ❌ NEW BUILD | Loopix-style constant-rate background traffic between paired devices so message timing is unobservable. Idle-slot padding wraps real messages in chaff. Only between pinned contacts; self-mesh skips. |
| 7 | **Hardware-attested transport** | 🟡 QUIC shipped Phase A2; pluggable transport NEW | QUIC over TLS 1.3, peer fingerprint pinned via hardware-bound key signature. Pluggable transport (Cloak/Obfs4 patterns) so DPI sees what looks like normal HTTPS. Censorship-evading by default. |
...
| 9 | **Threshold recovery** | 🟡 PARTIAL (harvest OneField privacy/sharding.cl) | Shamir(3,5) of identity secret distributed across trusted contacts. Lose 2 of 5 trusted friends, still recoverable. No password resets, no central recovery agent. OneField crate is Tier 15 production. |

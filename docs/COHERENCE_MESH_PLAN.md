...
| 7 | **Hardware-attested transport** | 🟡 PARTIAL — QUIC shipped Phase A2; obfs primitive shipped 2026-05-13 (`ol_onion::transport_obfs`); full DPI-mimicry handshake NEW | QUIC over TLS 1.3, peer fingerprint pinned via hardware-bound key signature. Pluggable transport (Cloak/Obfs4 patterns) so DPI sees what looks like normal HTTPS. The obfuscation primitive (ChaCha20 byte XOR) is shipped; the TLS-shaped handshake on top is still TODO. |

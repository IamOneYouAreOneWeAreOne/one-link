...
VARIABLES
    ...
    repair_plan_log \* set of (chunk, device) assignment records

vars == << holders, online, repair_plan_log >>

------------------------------------------------------------------------
TypeOK ==
    /\ holders \in [Chunks -> SUBSET Devices]
    /\ online \subseteq Devices
    /\ repair_plan_log \subseteq (Chunks \X Devices)
...
DeviceGoesOffline(d) ==
    ...
    /\ online' = online \ {d}
    /\ UNCHANGED << holders, repair_plan_log >>
...
RepairAssign(c, d) ==
    ...
    /\ d \in online
    /\ d \notin holders[c]
    /\ <<c, d>> \notin repair_plan_log
    /\ Cardinality({entry \in repair_plan_log : entry[1] = c}) < (Cardinality(Devices) - Cardinality(holders[c]))
    /\ repair_plan_log' = repair_plan_log \cup {<<c, d>>}
    /\ UNCHANGED << holders, online >>

\* A device picks up an assigned chunk (the fetch completes).
DeviceAcquiresChunk(c, d) ==
    /\ <<c, d>> \in repair_plan_log
    /\ d \in online
    /\ holders' = [holders EXCEPT ![c] = holders[c] \cup {d}]
    /\ UNCHANGED << online, repair_plan_log >>
...
\* chunk. (Structural: repair_plan_log is a SET.)
NoDoubleAssignPerChunk ==
    \A c \in Chunks, d \in Devices :
        Cardinality({e \in repair_plan_log : e[1] = c /\ e[2] = d}) <= 1

\* The repair plan never assigns a chunk to a device that already
\* holds it.
NoAssignmentToExistingHolder ==
    \A entry \in repair_plan_log :
        LET c == entry[1]
            d == entry[2]
        IN d \notin holders[c]
======================================================================

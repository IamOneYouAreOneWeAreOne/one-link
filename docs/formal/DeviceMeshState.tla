...
CONSTANTS
    ...
    OpsPool,        \* finite in-flight slots bounding replay growth
...
VARIABLES
    network,        \* sequence of in-flight (device_id, seq) tuples
...
TypeOK ==
    /\ network \in Seq(Devices \X (1 .. MaxSeq))
    /\ Len(network) <= Cardinality(OpsPool)
    /\ seen \in [Devices -> SUBSET (Devices \X (1 .. MaxSeq))]
    /\ state_set \in [Devices -> SUBSET (Devices \X (1 .. MaxSeq))]
...
Quiesce ==
    /\ network = <<>>
    /\ \A d \in Devices : local_max_seq[d] = MaxSeq
    /\ UNCHANGED vars
...
\* Every applied operation names an emitter sequence that the emitter has
\* already generated. The attacker can replay/reorder/drop authenticated
\* messages, but cannot synthesize an unissued sequence.
SignatureRequired ==
    \A d \in Devices :
        \A op \in state_set[d] :
            /\ op[1] \in Devices
            /\ op[2] \in 1 .. local_max_seq[op[1]]

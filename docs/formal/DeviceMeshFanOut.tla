...
------------------------------------------------------------------------
\* This formula documents the desired liveness contract, but is deliberately
\* not claimed as verified in the adversarial configuration below. An attacker
\* allowed to drop deliveries forever can always prevent eventual completion;
\* proving liveness requires an explicit bounded-loss/fair-delivery assumption.
EventualCompletion ==
    Cardinality(failed) < (Cardinality(Sources) - K_THRESHOLD + 1) =>
        <>(Cardinality(completed) >= K_THRESHOLD)

(* Sources and chunks are opaque, interchangeable model values. *)
Symmetry == Permutations(Sources) \cup Permutations(Chunks)
======================================================================

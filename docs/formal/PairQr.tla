...
(*
  ...
  Verified properties:
    ...
    - SAS_AGREEMENT_ON_HONEST_RUN: if both sides reach Done, they hold
      the same chain key even while attacker actions are enabled.

  Verification: run TLC over a small finite state space (typically
  2 inviters, 1 scanner, 2 invites, 2 attacker actions). The spec is a
...
VARIABLES
    ...
    scanner_has_scanned, \* pre-constructor QR scan completed
    inviter_invite,   \* function Inviters -> the exact invite each issued
...
vars == <<inviter_state, scanner_state, scanner_has_scanned,
          inviter_invite, bound_invite, transcript, chain_key,
...
Init ==
    ...
    /\ scanner_has_scanned = [s \in Scanners |-> FALSE]
    /\ inviter_invite \in [Inviters -> Invites]
...
ScannerScan(s, i, inv) ==
    ...
    /\ ~scanner_has_scanned[s]
    /\ inv = inviter_invite[i]
    /\ scanner_has_scanned' =
           [scanner_has_scanned EXCEPT ![s] = TRUE]
    /\ bound_invite' = [bound_invite EXCEPT ![s] = inv]
    ...
    /\ UNCHANGED <<inviter_state, scanner_state, inviter_invite,
                   wire_confirms>>
...
InviterReceiveResponse(i, inv, s, resp_transcript) ==
    ...
    /\ UNCHANGED <<scanner_state, scanner_has_scanned,
                   inviter_invite, bound_invite,
                   wire_responses, wire_confirms>>
...
ScannerReceiveConfirm(s, i, t) ==
    ...
    /\ UNCHANGED <<inviter_state, scanner_has_scanned,
                   inviter_invite, bound_invite,
                   transcript, chain_key,
                   wire_responses, wire_confirms>>
...
AttackerInjectResponse ==
    ...
    /\ \E inv \in Invites :
         \E s \in Scanners :
           LET forged == <<"forged", inv, s>> IN
             /\ wire_responses' = wire_responses \cup {<<inv, s, forged>>}
             /\ UNCHANGED <<inviter_state, scanner_state,
                            scanner_has_scanned, inviter_invite, bound_invite,
                            transcript, chain_key, wire_confirms>>

AttackerInjectConfirm ==
    /\ AttackerOn
    /\ \E i \in Inviters :
         LET forged == <<"forged", i>> IN
           /\ wire_confirms' = wire_confirms \cup {<<i, forged>>}
           /\ UNCHANGED <<inviter_state, scanner_state,
                          scanner_has_scanned, inviter_invite, bound_invite,
                          transcript, chain_key, wire_responses>>
...
Next ==
    ...
    \/ \E i \in Inviters :
         \E inv \in Invites :
           \E s \in Scanners :
             \E t \in {TranscriptOf(i, s, inv)}
                         \cup {<<"forged", inv, s>>} :
               InviterReceiveResponse(i, inv, s, t)
    \/ \E i \in Inviters : InviterConfirm(i)
    \/ \E s \in Scanners :
         \E i \in Inviters :
           \E t \in {TranscriptOf(j, s, b) :
                        j \in Inviters, b \in Invites}
                     \cup {<<"forged", i>>} :
             ScannerReceiveConfirm(s, i, t)
...
NoUnverifiedConfirm ==
    \A s \in Scanners :
        scanner_state[s] = "Done" =>
            \E i \in Inviters :
                /\ scanner_has_scanned[s]
                /\ bound_invite[s] = inviter_invite[i]
...
StateTypesOk ==
    ...
    /\ scanner_has_scanned \in [Scanners -> BOOLEAN]

(* I4. SAS_AGREEMENT_ON_HONEST_RUN:
       Any verified pair that both reaches Done holds the same chain key,
       even while attacker injection actions are enabled. *)

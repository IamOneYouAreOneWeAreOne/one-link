...
| τ_c × homology coupling | Closing-loop fragility events from `ol_homology` source into reaction-diffusion `S` term; field anticipates partitions | **Met + wired: `inject_fragility_events` clamps S at affected nodes; field re-equilibrates around fragile region. `Daemon._observe_prefetch` populates `_chunk_holders` registry; `_field_homology_feeder_loop` ticks every 30s, builds the cohold graph, runs `homology_native.fragility_score`, translates to events, and calls `FieldSnapshotManager.update_fragility_events`. 5 acceptance tests in `test_phase_e_daemon_couplings.py` + 1 fragility-reduces-field test in `test_field_snapshot_manager.py`.** |
| τ_c × active-inference coupling | `ol_prefetch` cohort prior pre-positions chunks along high-τ_c paths before request | **Met + helper wired: `prefetch_priorities` ranks holders by log-deficit + field-distance cost; `Daemon.field_rank_holders(holders, requester_short_id=None)` is the daemon-facing call site (passes through to `field_score_for_peer`, falls back to input-order on no-snapshot / unknown-peer / `ONE_LINK_FIELD_PREFETCH_DISABLE=1`). Awaits the multi-holder swarm-fetch decision path itself (upstream feature). 5 acceptance tests in `test_phase_e_daemon_couplings.py`.** |

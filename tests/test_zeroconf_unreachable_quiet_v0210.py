...
def _emit(log, exc: BaseException) -> None:
    try:
        raise exc
    except BaseException:
        log.warning("Error with socket 23 (('192.168.64.10', 5353))): %s", exc,
                    exc_info=True)
...
def test_no_route_to_host_keeps_its_warning_but_loses_its_traceback(zc_logger, caplog):
    discovery._quiet_zeroconf_unreachable_interfaces()

    with caplog.at_level(logging.WARNING, logger="zeroconf"):
        _emit(zc_logger, OSError(errno.EHOSTUNREACH, "No route to host"))

...
def test_issue_and_verify_round_trip():
    m = _fresh_master()
    c = fresh_challenge_for_peer()
    doc = issue_for_challenge(m, c, now_unix=1_000)
    verify_doc(doc, c, now_unix=1_010)
...
def test_issue_with_field_witness_binds_doc():
    ...
    doc = issue_for_challenge(m, c, field_witness=witness, now_unix=1_000)
    # Verify with the right witness passes.
    verify_doc(doc, c, expected_field_witness=witness, now_unix=1_010)
    # Wrong witness rejected.
    with pytest.raises(ValueError):
        verify_doc(
            doc, c, expected_field_witness=bytes([0xCD] * 32), now_unix=1_010
        )
...
def test_wrong_challenge_rejected():
    ...
    c1 = fresh_challenge_for_peer()
    c2 = fresh_challenge_for_peer()
    doc = issue_for_challenge(m, c1, now_unix=1_000)
    with pytest.raises(ValueError):
        verify_doc(doc, c2, now_unix=1_010)
...
def test_expired_doc_rejected():
    ...
    with pytest.raises(ValueError):
        verify_doc(doc, c, now_unix=1_100)
...
def test_wire_dict_round_trip_through_json():
    ...
    verify_doc(doc2, c, now_unix=1_010)
